...
to remember the order of arguments is to think of the function as as
constructing an operator that is then applied to the quantity.
...
`quax`'s flexible and extensible system to enable in for custom array-ish
objects, like `Quantity`, in `JAX`. Also, `unxt` uses multiple dispatch to
enable deep interoperability between `unxt` and other libraries, like `astropy`,
`gala` (, and anything user-defined).
