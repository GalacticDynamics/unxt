# About unxt's API conventions

`unxt` makes a handful of choices that show up everywhere in the API — a functional form alongside every method, a particular argument order, multiple dispatch at the core. None of them are arbitrary, and knowing the reasoning makes the rest of the API predictable.

## Naming

`unxt` uses a few conventions to make the code more readable and to avoid verbosity. Many of these are also found in the {doc}`glossary <../reference/glossary>`.
...
## Why there are two APIs for everything
...
- {doc}`../reference/glossary` — the vocabulary these conventions use.
- {doc}`why-quantity-is-non-parametric` — the biggest of these design choices.
- {doc}`../about/contributing` — the conventions as a contributor checklist.
