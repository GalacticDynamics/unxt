...
USys
  A shorthand for "unit system", used in class names to avoid verbosity.
...
Non-parametric Class
  ...
  The `unxt.Distance` class is an example of a non-parametric class; it has pre-fixed dimensions and does not require `unxt.Quantity`'s dimension parameter.
