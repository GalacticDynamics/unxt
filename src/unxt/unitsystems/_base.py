...
from ._utils import get_dimension_name, is_annotated
from unxt._typing import Unit as UnitT
...
class AbstractUnitSystem:
    ...
    def __init_subclass__(cls) -> None:
        ...
        if dimensions in _UNITSYSTEMS_REGISTRY:
            ...
            if is_dataclass(cls) and "__slots__" in cls.__dict__:
                return
        ...
        # If `make_dataclass(slots=True)` then the class is made twice, the
        # second time adding the `__slots__` attribute
        if is_dataclass(cls) and "__slots__" not in cls.__dict__:
            return
