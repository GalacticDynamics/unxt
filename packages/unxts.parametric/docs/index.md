...
Whether the dimension type parameter (e.g. `['length']`) appears in a `ParametricQuantity`'s `repr()` / `str()` is controlled by `unxts.parametric.config` — the parametric counterpart to `unxt.config`. By default the parameter is hidden in `repr()` and shown in `str()`:
...
The other display settings (`short_arrays`, `use_short_name`, `named_unit`, `indent`) are shared with all quantities and remain in `unxt.config`; see the [Configuration guide](../../guides/configuration.md).
