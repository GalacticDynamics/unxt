...
def wrap_to(
    draw: st.DrawFn,
    quantity: u.AbstractQuantity | st.SearchStrategy[u.AbstractQuantity],
    /,
...
_wrap_to_strategy = wrap_to

WrapToArgs: TypeAlias = (
    tuple[u.Q | st.SearchStrategy[u.Q], u.Q | st.SearchStrategy[u.Q]] | None
)
...
def angles(
    draw: st.DrawFn,
    *,
    wrap_to: WrapToArgs | st.SearchStrategy[WrapToArgs] = None,  # pylint: disable=redefined-outer-name
    **kwargs: Any,
) -> u.Angle:
    ...
    **kwargs
    ...
    angle_strategy = quantities(unit, quantity_cls=u.Angle, **kwargs)

    # Apply wrapping if bounds are provided
    if wrap_to is not None:
        wrap_min, wrap_max = wrap_to
        return draw(_wrap_to_strategy(angle_strategy, min=wrap_min, max=wrap_max))
